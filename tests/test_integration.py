...
class TestEnhancedIntegration(unittest.TestCase):
    def test_enhanced_detection(self):
        ...
        print(f"AWS patterns loaded: {len(pattern_library.categories.get('aws', {}).patterns)}")
        ...
        print(f"Test file exists: {os.path.exists('tests/testdata/enhanced/aws_credentials.txt')}")
        ...
        for i, finding in enumerate(findings):
            print(f"Finding {i+1}: {finding.get('pattern_category', 'NO CATEGORY')} - {finding.get('value', 'NO VALUE')}")
        ...
        self.assertTrue(any(finding.get("pattern_category") == "aws" for finding in findings))