...
class TestFullPipelineScan:
    ...
    def test_finds_aws_key(self):
        rc, output = _run_scan(LEAKY_REPO)
        assert (
            "AKIA" in output or "aws" in output.lower()
    ...
    def test_finds_terraform_credential(self):
        ...
        assert (
            ...
            or "main.tf" in output.lower()
            or "TerraformProdPass" in output
            or "infra" in output.lower()
        ), f"Expected Terraform finding.\nOutput:\n{output}"

    def test_finds_cicd_secret(self):
        rc, output = _run_scan(LEAKY_REPO)
        assert (
            "deploy.yml" in output
            or "cicd" in output.lower()
            or "API_SECRET" in output
            or "workflows" in output.lower()
        ), f"Expected CI/CD finding.\nOutput:\n{output}"

    def test_finds_dockerfile_credential(self):
        rc, output = _run_scan(LEAKY_REPO)
        assert (
            "Dockerfile" in output
            or "docker" in output.lower()
            or "DB_PASSWORD" in output
    ...
    def test_clean_directory_exits_with_code_0(self, tmp_path):
        ...
        assert (
            rc == 0
        ), f"Expected exit code 0 for clean dir, got {rc}.\nOutput:\n{output}"


class TestOutputFormats:
    def test_json_output_produced(self, tmp_path):
        rc, output = _run_scan(
            LEAKY_REPO, ["--output", "json", "--output-dir", str(tmp_path)]
        )
    ...
    def test_sarif_output_produced(self, tmp_path):
        rc, output = _run_scan(
            LEAKY_REPO, ["--output", "sarif", "--output-dir", str(tmp_path)]
        )
...
class TestNewFlags:
    def test_no_context_analysis_flag_works(self):
        ...
        assert (
            rc == 1
        ), f"Expected findings even without context analysis.\nOutput:\n{output}"
