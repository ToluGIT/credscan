...
class WebCrawler:
    ...
    def _is_same_domain(self, url: str, base_domain: str) -> bool:
        ...
        try:
            ...
            return parsed_url.netloc == base_domain or parsed_url.netloc.endswith(
                f".{base_domain}"
            )
        except:
            return False
    ...
    def crawl_links(self, base_url: str, max_depth: int = None) -> Set[str]:
        ...
        while urls_to_visit:
            ...
            try:
                response = requests.get(
                    current_url, timeout=self.timeout, headers=headers
                )
                ...
                for link in soup.find_all(["a", "link", "script", "img", "iframe"]):
                    ...
                    if (
                        not self._is_same_domain(full_url, base_domain)
                        or "#" in full_url
                        or full_url.startswith("mailto:")
                        or full_url.startswith("javascript:")
                        or full_url.startswith("tel:")
                    ):
                        continue
        ...
        logger.info(
            f"Crawled {len(visited_urls)} pages, found {len(discovered_urls)} interesting files"
        )
    ...
    def discover_urls(
        self, base_url: str, use_crawling: bool = True, use_wordlists: bool = True
    ) -> Set[str]:
        """
