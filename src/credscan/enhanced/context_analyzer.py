...
class ContextAnalyzer:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.context_window_size = self.config.get('context_window_size', 5)  # Lines before/after
            ...
            for context_name, context_data in contexts.items():
                ...
                for pattern in patterns:
                    try:
                        ...
                        logger.warning(f"Invalid regex pattern '{pattern}' in {context_category}.{context_name}: {e}")
                
                compiled[context_category][context_name] = compiled_patterns
        
        return compiled
    
    def analyze_context(self, file_path: str, content: str, finding_line: int, 
                       finding_key: str = None, finding_value: str = None) -> Dict[str, Any]:
        """
        ...
        context_result = {
            ...
            'config_context': self._analyze_config_context(content, finding_line, file_path),
        ...
        context_result['confidence_modifier'] = self._calculate_confidence_modifier(context_result)
    ...
    def _analyze_file_context(self, file_path: str) -> Dict[str, Any]:
        ...
        for context_name, context_data in self.context_patterns.get('file_contexts', {}).items():
            patterns = context_data.get('patterns', [])
        ...
        if file_extension in special_files or any(sf in filename for sf in special_files.keys()):
    ...
    def _analyze_code_context(self, content: str, finding_line: int) -> Dict[str, Any]:
        ...
        for context_name, patterns in self.compiled_patterns.get('code_contexts', {}).items():
        ...
        return {
            ...
            'line_content': lines[finding_line - 1] if finding_line <= len(lines) else ''
        }
    
    def _analyze_config_context(self, content: str, finding_line: int, file_path: str) -> Dict[str, Any]:
        """Analyze configuration file context."""
        detected_contexts = []
        confidence_modifiers = []
        
        # Only analyze if this looks like a configuration file
        config_extensions = {'.yaml', '.yml', '.json', '.toml', '.ini', '.conf', '.cfg', '.env'}
        file_extension = Path(file_path).suffix.lower()
        
        if file_extension not in config_extensions and 'config' not in file_path.lower():
            return {'detected_contexts': detected_contexts, 'confidence_modifiers': confidence_modifiers}
        ...
        context_start = max(0, finding_line - 10)  # Look further back for config structure
        context_lines = lines[context_start:finding_line + 5]
        context_text = '\n'.join(context_lines)
        
        # Check config context patterns
        for context_name, patterns in self.compiled_patterns.get('config_contexts', {}).items():
    ...
    def _analyze_api_context(self, content: str, finding_line: int) -> Dict[str, Any]:
        ...
        for context_name, patterns in self.compiled_patterns.get('api_contexts', {}).items():
    ...
    def _analyze_framework_context(self, content: str, file_path: str) -> Dict[str, Any]:
        """Analyze framework-specific context."""
        detected_contexts = []
        confidence_modifiers = []
        
        # Check framework context patterns
        for context_name, patterns in self.compiled_patterns.get('framework_contexts', {}).items():
            ...
            found_in_path = any(pattern.search(file_path.lower()) for pattern in patterns if hasattr(pattern, 'search'))
    ...
    def _calculate_confidence_modifier(self, context_result: Dict[str, Any]) -> float:
        ...
        for context_type in ['file_context', 'code_context', 'config_context', 'api_context', 'framework_context']:
        ...
        if len(sorted_modifiers) == 1:
            ...
            weights = [0.5, 0.3] + [0.2 / (len(sorted_modifiers) - 2)] * (len(sorted_modifiers) - 2)
            final_modifier = sum(mod * weight for mod, weight in zip(sorted_modifiers, weights))
        
        # Clamp to reasonable bounds
        return max(self.min_confidence_threshold, min(self.max_confidence_multiplier, final_modifier))
    ...
    def _determine_risk_level(self, context_result: Dict[str, Any]) -> str:
        ...
        if 'production_indicators' in file_contexts:
            ...
            return 'medium'
        elif 'test_indicators' in file_contexts or 'development_indicators' in file_contexts:
            return 'low'
    ...
    def enhance_finding(self, finding: Dict[str, Any], context_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        enhanced_finding.update({
            ...
            'context_analysis': {
                ...
                'detected_contexts': self._flatten_detected_contexts(context_analysis),
                'confidence_reasoning': self._generate_confidence_reasoning(context_analysis)
            }
        })
    ...
    def _adjust_severity(self, original_severity: str, risk_level: str, confidence: float) -> str:
