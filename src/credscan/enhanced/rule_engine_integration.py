...
class EnhancedRule(Rule):
    ...
    def __init__(self, rule_config: Dict[str, Any], pattern_library: Optional[PatternLibrary] = None):
        """
        ...
        self.enabled_categories: Set[str] = set(rule_config.get("enabled_categories", []))
                ...
                for category_name, patterns in matched_patterns.items():
                    for pattern in patterns:
                        ...
                        findings.append({
                            ...
                            "line": line,
                            "path": filepath,
                            "description": pattern.description or f"Detected {pattern.name} in {category_name} category"
                        })
    ...
    def register_enhanced_rules(self, pattern_library_path: Optional[str] = None):
        ...
        if pattern_library_path:
            try:
                ...
                self.logger.error(f"Error loading pattern library from {pattern_library_path}: {e}")
        ...
        self.logger.info(f"Registered {len(enhanced_rules)} enhanced detection rules with {len(pattern_library.get_all_patterns())} patterns")
...
class EnhancedRuleLoader:
    ...
    @staticmethod
    def load_default_rules(pattern_library: Optional[PatternLibrary] = None) -> List[EnhancedRule]:
        """
