...
def load_default_patterns() -> PatternLibrary:
                    ...
                    category.add_pattern(
                        CredentialPattern(
                            ...
                            severity=(
                                "high"
                                if any(
                                    x in pattern_keyword.lower()
                                    for x in ["secret", "key", "token", "password"]
                                )
                                else "medium"
                            ),
                            confidence=0.8,
    ...
    payment_category.add_pattern(
        CredentialPattern(
            ...
            examples=[
                ...
                "rk_test_1234567890123456789012",
            ],
        )
    )
    ...
    messaging_category.add_pattern(
        CredentialPattern(
    ...
    token_patterns = PatternCategory(
        name="token_patterns", description="Token credentials and secrets"
    )

    token_patterns.add_pattern(
        CredentialPattern(
            name="Generic Token",
    ...
    key_patterns = PatternCategory(
        name="key_patterns", description="Key credentials and secrets"
    )

    key_patterns.add_pattern(
        CredentialPattern(
            name="AWS API Key",
    ...
    secret_patterns = PatternCategory(
        name="secret_patterns", description="Secret credentials and secrets"
    )

    secret_patterns.add_pattern(
        CredentialPattern(
            name="Client Secret",
    ...
    password_patterns = PatternCategory(
        name="password_patterns", description="Password credentials and secrets"
    )

    password_patterns.add_pattern(
        CredentialPattern(
...
def load_patterns_from_file(filepath: str) -> PatternLibrary:
    ...
    if filepath.lower().endswith(".yaml") or filepath.lower().endswith(".yml"):
        ...
        raise ValueError(
            f"Unsupported file format: {filepath}. Only YAML and JSON are supported."
        )
...
def merge_pattern_libraries(
    base_library: PatternLibrary, additional_library: PatternLibrary
) -> PatternLibrary:
    ...
    for category_name, category in additional_library.categories.items():
        if category_name in merged_library.categories:
            ...
            if (
                ...
                and category_name in merged_library.enabled_categories
            ):
