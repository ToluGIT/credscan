...
class TechnologyDetector:
    ...
    def _matches_file_pattern(
        self, file_path: str, filename: str, pattern: str
    ) -> bool:
    ...
    def _detect_from_file_extension(self, file_path: str) -> Set[str]:
        ...
        # Kubernetes files
        if extension in [".yaml", ".yml"] and any(
            k8s in file_path.lower()
            for k8s in ["k8s", "kubernetes", "helm", "kustomize"]
        ):
    ...
    def get_relevant_patterns(self, file_path: str) -> Dict[str, List[re.Pattern]]:
        ...
        if "General Credentials" in self.compiled_patterns:
            relevant_patterns["General Credentials"] = self.compiled_patterns[
                "General Credentials"
            ]
    ...
    def scan_with_technology_context(
                    ...
                    for match in pattern.finditer(line):
                        ...
                        finding = {
                            ...
                            "severity": self._calculate_severity(
                                tech_category, variable_name, value
                            ),
                            ...
                            "confidence": self._calculate_confidence(
                                tech_category, variable_name, value, file_path
                            ),
                        }
    ...
    def _calculate_severity(
        self, tech_category: str, variable_name: str, value: str
    ) -> str:
    ...
    def _calculate_confidence(
        self, tech_category: str, variable_name: str, value: str, file_path: str
    ) -> float:
        ...
        if any(
            pattern in variable_name.lower() for pattern in high_confidence_patterns
        ):
            confidence += 0.1
...
class TechnologyAwareEngine:
    ...
    def scan(self) -> List[Dict[str, Any]]:
        ...
        for file_path in files_to_scan:
            try:
                ...
                file_findings = self.tech_detector.scan_with_technology_context(
                    file_path, content
                )
                ...
                logger.debug(
                    f"Error scanning {file_path} with technology detector: {e}"
                )
                continue
    ...
    def scan_file(self, filepath: str):
        ...
        try:
            ...
            tech_findings = self.tech_detector.scan_with_technology_context(
                filepath, content
            )
