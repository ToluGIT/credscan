...
class ContextAwareEngine:
    ...
    def __init__(self, base_engine, config: Dict[str, Any] = None):
        ...
        self.context_confidence_threshold = self.config.get(
            "context_confidence_threshold", 0.1
        )
        self.enable_context_filtering = self.config.get(
            "enable_context_filtering", True
        )

        # Confidence scoring settings
        self.enable_confidence_scoring = self.config.get(
        ...
        if self.enable_confidence_scoring:
            ...
            if "confidence_factor_weights" in self.config:
                confidence_config["factor_weights"] = self.config[
        ...
        logger.debug(
            f"ContextAwareEngine initialized with context analysis: {self.enable_context_analysis}, "
    ...
    def scan(self) -> List[Dict[str, Any]]:
        ...
        file_paths_with_findings = {
            f.get("path") for f in base_findings if f.get("path")
        }
        ...
        for finding in base_findings:
            enhanced_finding = self._enhance_finding_with_context(
                finding, file_contents
            )

            # Apply confidence-based filtering
            should_include = True
            context_confidence = enhanced_finding.get(
                "confidence", 1.0
            )  # Default to high if missing
            overall_confidence = enhanced_finding.get(
                "overall_confidence", context_confidence
            )  # Fallback to context confidence

            # Only filter if we have valid confidence scores and they're genuinely low
            if (
                self.enable_context_filtering
                and context_confidence < self.context_confidence_threshold
            ):
                should_include = False
                logger.debug(
                    f"Filtered out by context confidence: {enhanced_finding.get('value', '')[:30]}..."
                )

            # For overall confidence, be less aggressive - if context analysis failed, don't over-filter
            if (
                self.enable_confidence_scoring
                and overall_confidence > 0
                and overall_confidence < self.min_confidence_threshold
            ):
                should_include = False
                logger.debug(
                    f"Filtered out by overall confidence: {enhanced_finding.get('value', '')[:30]}..."
                )
        ...
        logger.debug(
            f"Context analysis completed. {len(enhanced_findings)} findings after context filtering"
        )
        ...
        if self.enable_deduplication and self.deduplicator and enhanced_findings:
            ...
            enhanced_findings = self.deduplicator.deduplicate_findings(
                enhanced_findings
            )
            logger.debug(
                f"After deduplication: {len(enhanced_findings)} unique findings"
            )
    ...
    def scan_file(self, filepath: str) -> List[Dict[str, Any]]:
        ...
        for finding in base_findings:
            ...
            if (
                self.enable_context_filtering
                and enhanced_finding.get("confidence", 0)
                < self.context_confidence_threshold
            ):
                should_include = False

            if (
                self.enable_confidence_scoring
                and enhanced_finding.get("overall_confidence", 0)
    ...
    def _enhance_finding_with_context(
        self, finding: Dict[str, Any], file_contents: Dict[str, str]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            enhanced_finding = self.context_analyzer.enhance_finding(
            ...
            if self.enable_confidence_scoring and self.confidence_scorer:
                ...
                enhanced_finding["confidence_explanation"] = confidence_result[
                    "explanation"
                ]

                if self.show_confidence_details:
                    enhanced_finding["confidence_breakdown"] = confidence_result[
                        "score_breakdown"
                    ]

                logger.debug(
                    f"Enhanced finding with confidence: {confidence_result['confidence']:.3f}"
                )

            logger.debug(
                f"Enhanced finding with context: {context_analysis['context_type']} "
            ...
            logger.debug(
                f"Error in context analysis for {file_path}:{line_number}: {e}"
            )
                ...
                try:
                    ...
                    confidence_result = (
                        self.confidence_scorer.calculate_confidence_score(
                            ...
                            None,
                            None,
                            None,  # No context/entropy/tech analysis
                        )
                    )

                    enhanced_finding = finding.copy()
                    enhanced_finding["overall_confidence"] = confidence_result[
                        "confidence"
                    ]
                    enhanced_finding["confidence_factors"] = confidence_result[
                        "factors"
                    ]
                    enhanced_finding["confidence_explanation"] = confidence_result[
                        "explanation"
    ...
    def _generate_context_aware_description(
        ...
        context_suffix = context_descriptions.get(
            context_type, f"in {context_type} context"
        )
