...
class ResultDeduplicator:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.compiled_test_patterns = [
            re.compile(pattern) for pattern in self.test_patterns
        ]
            ...
            enhanced.update(
                {
                    ...
                    "severity": self._adjust_severity_for_test(
                        enhanced.get("severity", "medium")
                    ),
    ...
    def _merge_findings(self, findings: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        base_finding = max(
            findings, key=lambda f: f.get("overall_confidence", f.get("confidence", 0))
        )
        ...
        if variable:
            for pattern in self.compiled_test_patterns:
                if pattern.search(variable):
                    indicators.append(
                        f"test_pattern_in_variable: {pattern.pattern[:30]}..."
                    )
                    confidence += 0.2
                    break
    ...
    def _is_secret_reference(self, value: str) -> bool:
        ...
        if re.match(
            r"(?i)(AKIA|ASIA|hf_|sk-|sk_|rk_|ghp_|gho_|ghu_|ghs_|ghr_|xox[baprs]-|AIza|glpat-|eyJ)",
            bare,
        ):
            return False
        ...
        if re.match(r"^[A-Za-z_][A-Za-z0-9_]*\s*[(\[]", bare) or re.match(
            r"^[A-Za-z_][A-Za-z0-9_]*\.[A-Za-z_][A-Za-z0-9_]*\s*\(", bare
        ):
            return True
        ...
        if re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", bare) and not re.search(
            r"[0-9]", bare
        ):
            return True
