...
class CICDParser(BaseParser):
    ...
    def can_parse(self, filepath: str) -> bool:
        ...
        if _GITHUB_ACTIONS_DIR.replace('\\', '/') in norm and basename.endswith(('.yml', '.yaml')):
            return True
    ...
    def _detect_platform(self, filepath: str, content: str = '') -> str:
        ...
        if content and self._GH_ACTIONS_CONTENT.search(content) and 'runs-on' in content:
            return 'github_actions'
        return 'unknown'
    ...
    def _find_token_curl_exposure(self, content: str, filepath: str) -> List[Dict[str, Any]]:
        entries = []
