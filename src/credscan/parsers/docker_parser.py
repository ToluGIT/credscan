...
_DOCKERFILE_NAMES = {'dockerfile', 'dockerfile.dev', 'dockerfile.prod', 'dockerfile.test'}
...
class DockerParser(BaseParser):
    ...
    def _scan_manifest(self, outer: tarfile.TarFile, filepath: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for env_entry in env_vars:
                ...
                if _is_credential_key(key) and not _is_placeholder(value) and len(value) >= 6:
                    entries.append({
                        'key': key,
                        'value': value,
                        'line': 0,
    ...
    def _scan_layers(self, outer: tarfile.TarFile, filepath: str) -> List[Dict[str, Any]]:
