...
class BinaryFileParser:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.max_extraction_size = self.config.get(
            "max_extraction_size", 100 * 1024 * 1024
        )  # 100MB
        ...
        self.extraction_timeout = self.config.get(
    ...
    def parse(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                or file_ext in self.application_extensions
            ):
            ...
            for file in files:
                ...
                for parser in parsers:
                    if parser.can_parse(file_path):
                        try:
                            logger.debug(
                                f"Using {parser.__class__.__name__} for {relative_path}"
                            )
                            parsed_result = parser.parse(file_path)

                            if parsed_result and parsed_result.get("items"):
                                logger.debug(
                                    f"Found {len(parsed_result['items'])} items in {relative_path}"
                                )
                                ...
                                for item in parsed_result["items"]:
                                    ...
                                    logger.debug(
                                        f"Extracted item: key='{item.get('key')}' value='{str(item.get('value', ''))[:30]}...'"
                                    )
                            ...
                            logger.warning(
                                f"Parser {parser.__class__.__name__} failed for {file_path}: {e}"
                            )
                            continue
                ...
                if not any(p.can_parse(file_path) for p in parsers):
                    logger.debug(
                        f"No parser available for extracted file: {relative_path}"
                    )
    ...
    def _parse_database(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for (table_name,) in tables:
                ...
                might_have_creds = any(
                    keyword in table_lower for keyword in credential_tables
                )
    ...
    def _parse_certificate(self, file_path: str) -> Dict[str, Any]:
        ...
        if not HAS_CRYPTOGRAPHY:
            return self._create_error_result(
                "cryptography library required for certificate parsing"
            )
            ...
            for password in common_passwords:
                try:
                    private_key, certificate, additional_certificates = (
                        pkcs12.load_key_and_certificates(p12_data, password)
                    )
    ...
    def _create_error_result(self, error_message: str) -> Dict[str, Any]:
        ...
        return {"type": "binary_error", "items": [], "error": error_message}
