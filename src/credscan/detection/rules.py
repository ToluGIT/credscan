...
class Rule:
    ...
    def __init__(self, rule_config: Dict[str, Any]):
        ...
        if (
            "variable_exclusion_pattern" in rule_config
            and rule_config["variable_exclusion_pattern"]
        ):
            try:
        ...
        for pattern_def in rule_config.get("value_patterns", []):
            try:
                ...
                logger.error(
                    f"Invalid regex in rule {self.id} value pattern: {pattern_def.get('pattern')} - {e}"
                )
        ...
        for pattern in rule_config.get("value_exclusion_patterns", []):
            try:
                ...
                logger.error(
                    f"Invalid regex in rule {self.id} value exclusion: {pattern} - {e}"
                )
    ...
    def is_variable_name_match(self, var_name: str) -> bool:
        ...
        if self.variable_exclusion_pattern and self.variable_exclusion_pattern.search(
            var_name
        ):
            return False
    ...
    def apply(
        ...
        for item in items:
            ...
            if (
                key
                and self.is_variable_name_match(key)
                and not self.is_excluded_value(value)
            ):
                findings.append(
                    {
                        "rule_id": self.id,
        ...
        default_rule_configs = [
            ...
            },
            {
                ...
                "value_patterns": [
                    ...
                    {"name": "Bcrypt Hash", "pattern": r"^\$2[ayb]\$.{56,57}$"},
                    {
                        "name": "AWS Client ID",
            ...
            },
            {
                ...
                "severity": "medium",
                "value_patterns": [
                    {"name": "Password Content", "pattern": r"(?i)\w*password[_-]?\w*"},
                    {"name": "Token Content", "pattern": r"(?i)\w*token[_-]?\w*"},
                    {
                        "name": "API Key Content",
                        "pattern": r"(?i)\w*api[_-]?key[_-]?\w*",
                    },
                    {"name": "Secret Content", "pattern": r"(?i)\w*secret[_-]?\w*"},
                ],
                "value_exclusion_patterns": [
