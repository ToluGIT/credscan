...
class BaselineManager:
    ...
    def __init__(self, baseline_file: Optional[str] = None):
        ...
        self.exclusions = {"findings": [], "patterns": [], "paths": []}
    ...
    def load_baseline(self) -> bool:
        ...
        try:
            ...
            self.exclusions = data.get(
                "exclusions", {"findings": [], "patterns": [], "paths": []}
            )
    ...
    def save_baseline(self, metadata: Dict[str, Any] = None) -> bool:
        ...
        try:
            ...
            baseline = {"version": 1, "metadata": meta, "exclusions": self.exclusions}

            # Ensure directory exists
            os.makedirs(
                os.path.dirname(os.path.abspath(self.baseline_file)), exist_ok=True
            )
    ...
    def is_excluded(self, finding: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for excluded in self.exclusions.get("findings", []):
            if (
                excluded.get("path") == path
                and excluded.get("line") == line
                and excluded.get("value_hash") == value_hash
            ):
                return {
                    "excluded": True,
    ...
    def remove_exclusion(self, exclusion_id: str) -> bool:
        ...
        for i, excl in enumerate(self.exclusions["patterns"]):
            if excl.get("id") == exclusion_id:
                ...
                # Update compiled regexes
                self.pattern_regexes = [
                    (id, p) for id, p in self.pattern_regexes if id != exclusion_id
                ]
                return True
        ...
        for i, excl in enumerate(self.exclusions["paths"]):
            if excl.get("id") == exclusion_id:
                ...
                # Update compiled regexes
                self.path_regexes = [
                    (id, p) for id, p in self.path_regexes if id != exclusion_id
                ]
                return True
