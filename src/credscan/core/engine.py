    ...
    def find_files(self) -> List[str]:
        ...
        if os.path.isfile(self.scan_path):
            ...
            for root, dirs, files in os.walk(self.scan_path):
                ...
                dirs[:] = [
                    d for d in dirs if self.should_scan_file(os.path.join(root, d))
                ]
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for future in concurrent.futures.as_completed(future_to_file):
                ...
                try:
                    ...
                    if findings and self.baseline_manager:
                        ...
                        for finding in findings:
                            ...
                            if exclusion.get("excluded"):
                                ...
                                finding["exclusion_reason"] = exclusion.get(
                                    "reason", "Unknown"
                                )
                    ...
                    if (
                        self.files_scanned % 100 == 0
                        or self.files_scanned == self.files_found
                    ):
                        logger.info(
                            f"Scanned {self.files_scanned}/{self.files_found} files"
                        )
