...
def parse_args():
    ...
    target.add_argument(
        "--url", metavar="URL", type=str, help="Web URL to scan for credentials"
    )
    target.add_argument(
        "--crawl",
        action="store_true",
    ...
    detect.add_argument(
        "--no-entropy", action="store_true", help="Disable all entropy-based detection"
    )
    detect.add_argument(
        "--no-context-analysis",
        action="store_true",
    ...
    git.add_argument(
        "--until", metavar="DATE", help="Only scan commits older than DATE"
    )
    git.add_argument(
        "--branch",
        metavar="REF",
    ...
    adv.add_argument(
        "--no-enhanced-entropy", action="store_true", help=argparse.SUPPRESS
    )
    adv.add_argument("--no-binary-parsing", action="store_true", help=argparse.SUPPRESS)
    adv.add_argument(
        "--no-confidence-scoring", action="store_true", help=argparse.SUPPRESS
    )
    adv.add_argument(
    ...
    adv.add_argument(
        "--confidence-weights", metavar="JSON_OR_FILE", help=argparse.SUPPRESS
    )
    adv.add_argument(
        "--binary-max-size", metavar="MB", type=int, default=100, help=argparse.SUPPRESS
    )
    adv.add_argument(
        "--web-timeout", metavar="SEC", type=int, default=10, help=argparse.SUPPRESS
    )
    adv.add_argument(
        "--crawl-delay", metavar="SEC", type=float, default=1.0, help=argparse.SUPPRESS
    )
...
def build_engine(args, config, base_engine):
    ...
    if args.pattern_categories:
        enhanced_config_data["enabled_pattern_categories"] = (
            args.pattern_categories.split(",")
        )
    ...
    enhanced_config_data["context_confidence_threshold"] = (
        args.context_confidence_threshold
    )
    ...
    if args.confidence_weights:
        ...
        try:
            if args.confidence_weights.startswith("{"):
                enhanced_config_data["confidence_factor_weights"] = json.loads(
                    args.confidence_weights
                )
            else:
...
""")
    ...
    if hasattr(args, "url") and args.url:
        ...
        try:
            from credscan.web import WebCrawler, WebScanner
        except ImportError:
            logger.error(
                "Web scanning requires the 'requests' package. "
        ...
        if hasattr(args, "crawl") and args.crawl:
            ...
            discovered_urls = web_crawler.discover_urls(
                args.url, use_crawling=True, use_wordlists=True
            )
    ...
    if args.legacy_patterns:
        rules = (
            RuleLoader.load_rules_from_file(args.rules)
            if args.rules
            else RuleLoader.load_default_rules()
        )
    ...
    if getattr(args, "verify", False) and findings:
        logger.info(
            "Verifying discovered tokens against provider identity endpoints..."
        )
    ...
    if args.exclude_pattern and args.baseline_file:
        if engine.baseline_manager:
            try:
                engine.baseline_manager.add_pattern_exclusion(
                    args.exclude_pattern, args.exclusion_reason
                )
    ...
    if args.exclude_path and args.baseline_file:
        if engine.baseline_manager:
            try:
                engine.baseline_manager.add_path_exclusion(
                    args.exclude_path, args.exclusion_reason
                )
                engine.baseline_manager.save_baseline()
    ...
    statistics = {
        ...
        "findings_count": len(findings),
        "excluded_count": (
            len(engine.excluded_findings) if hasattr(engine, "excluded_findings") else 0
        ),
    }
