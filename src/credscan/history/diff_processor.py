...
class DiffProcessor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.parsers = [JSONParser(config), YAMLParser(config), CodeParser(config)]
        ...
        try:
            ...
            if change_type == "A":  # Added file
                ...
                if not added_content:
                    logger.warning(
                        f"No content found for new file {file_path} in commit {commit_hash}"
                    )
                    return []
                ...
                line_mapping = [
                    (i + 1, line) for i, line in enumerate(added_content.split("\n"))
                ]
                ...
                if not added_lines:
                    logger.warning(
                        f"No added lines found for {file_path} in commit {commit_hash}"
                    )
                    return []
            ...
            try:
                ...
                if not parsed_content or parsed_content.get("error"):
                    logger.warning(
                        f"Error parsing content: {parsed_content.get('error')}"
                    )
                    os.unlink(temp_path)  # Clean up temp file
                    return []
                ...
                for rule in self.rules:
                    ...
                    if rule_findings:
                        logger.debug(
                            f"Rule '{rule.name}' found {len(rule_findings)} findings"
                        )
                ...
                for i, finding in enumerate(findings):
                    logger.debug(
                        f"Finding {i+1}: {finding.get('variable', 'N/A')} = {finding.get('value', 'N/A')}"
                    )
                ...
                if self.baseline_manager:
                    ...
                    for finding in findings:
                        ...
                        if not exclusion.get("excluded", False):
                            ...
                            logger.debug(
                                f"Excluded finding: {finding.get('variable', 'N/A')} = {finding.get('value', 'N/A')}"
                            )
    ...
    def _get_change_type(self, commit_hash: str, file_path: str) -> str:
        ...
        try:
            ...
            logger.error(
                f"Error getting change type for {file_path} at {commit_hash}: {e}"
            )
            return "M"  # Default to modified
