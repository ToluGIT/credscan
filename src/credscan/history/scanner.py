        ...
        try:
            ...
            try:
                ...
                if self.max_commits and len(commits) > self.max_commits:
                    logger.info(
                        f"Limiting analysis to {self.max_commits} commits (out of {len(commits)} total)"
                    )
    ...
    def scan(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i, commit_hash in enumerate(commits):
                ...
                unique_findings = self.result_manager.process_commit_findings(
                    commit_hash, findings
                )
                ...
                if (i + 1) % 10 == 0:
                    logger.info(
                        f"Progress: {i+1}/{len(commits)} commits analyzed, {total_findings} findings so far"
                    )

            # Get final results
            results = self.result_manager.get_findings()
            logger.info(
                f"History scan complete. Found {len(results)} unique credential exposures across {len(commits)} commits."
            )
